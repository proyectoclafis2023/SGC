...
import { 
    ...
    BarChart3, Fingerprint, Database, Zap, RefreshCw, Layers, 
    Eye, EyeOff, Search, Globe, Heart, 
    TrendingUp, TrendingDown, Clock, Bell
} from 'lucide-react';
...
/**
 * SVG TREND CHART (SGC v3.4 — ROBUST)
 */
    ...
    const filteredChecks = useMemo(() => {
        ...
        return report.checks.map((check: Check) => {
            const issues = (check.issues || []).filter(issue => {
                ...
                return matchesSeverity && matchesSearch && !isHiddenInfo;
            });
            return { ...check, issues };
        });
    }, [report, showInfo, severityFilter, search]);
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700 pb-20 max-w-7xl mx-auto px-4 mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 ...
                 <div className="md:col-span-2 bg-white dark:bg-gray-900/40 p-8 rounded-[3rem] border border-gray-100 dark:border-gray-800 shadow-xl">
                    <h4 className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-6 flex items-center gap-2"><Bell className="w-3 h-3" /> Alertas del Sistema</h4>
                    <div className="flex flex-col gap-3">
                        {report.events?.map((ev: any, i: number) => (
                            <div key={i} className={`p-4 rounded-2xl border flex items-center justify-between ${getStatusColor(ev.severity)}`}>
                                <div className="flex items-center gap-3">
                                    <Zap className="w-4 h-4" />
                                    <span className="text-[9px] font-black uppercase">{ev.message}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {filteredChecks.map((check: any, idx: number) => (
                    <div key={idx} className="bg-white dark:bg-gray-900/40 p-10 rounded-[3.5rem] border border-gray-100 dark:border-gray-800">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                               <h3 className="text-lg font-black uppercase italic">{t(check.name.toLowerCase())}</h3>
                            </div>
                        ...
                        <div className="space-y-4">
                            {check.issues.length > 0 ? check.issues.map((issue: any, i: number) => (
                                <div key={i} className="p-6 bg-gray-50 dark:bg-gray-800/40 rounded-3xl border border-gray-100 dark:border-gray-800">
                                    <p className="text-xs font-bold opacity-90">{issue.message}</p>
                                    <div className="mt-2 text-[10px] font-medium text-emerald-600 italic">{issue.action}</div>
                                </div>
                            )) : (
